...
def _log_likelihood(params, *args):
    ...
    likelihood = np.dot(empirical_counts, params) - total_logZ - \
                 np.sum(np.dot(params,params))/(squared_sigma*2) # 正则化?
...
class LinearChainCRF():
    ...
    def print_test_result(self, test_corpus_filename, file):
        test_data = self._read_corpus(test_corpus_filename)

        for X, Y in test_data:
            Yprime = self.inference(X)
            for t in range(len(X)):
                text = '\t'.join(X[t]) + ' ' + Yprime[t]
                file.write(text)
                file.write('\n')
            file.write('\n')
...
# english
# crf.train('data/NER/English/train.txt', 'English5.json')
# crf.load('English3.json')
# crf.test('data/NER/English/validation.txt')
# filename = 'data/NER/English/English_my_result.txt'
# file = open(filename, 'w')
# crf.print_test_result('data/NER/English/validation.txt', file)

# chinese
# crf.train('data/NER/Chinese/train.txt', 'Chinese2.json')
